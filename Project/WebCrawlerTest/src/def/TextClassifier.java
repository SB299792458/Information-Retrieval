...
public class TextClassifier {
    ...
    public static void main(String args[])throws Exception {
        ...
        BufferedReader spamD=new BufferedReader(new FileReader("SpamText.txt"));
        String spm=spamD.readLine();
		while(spm!=null)
		{
			listText.add(spm);
			listClasses.add("spam");
			spm=spamD.readLine();
		}
		BufferedReader normD=new BufferedReader(new FileReader("NormText.txt"));
        String nrm=normD.readLine();
		while(nrm!=null)
		{
			listText.add(nrm);
			listClasses.add("no spam");
			nrm=normD.readLine();
		}
        
		String[] inputText= new String[listText.size()];
		Object[] obj = (Object[]) listText.toArray();
		for(int i=0; i<obj.length;i++) {
			inputText[i] = (String)obj[i];
		}
      // String[] inputText =  {"hey, buy this from me!", "do you want to buy?", "I have a party tonight!", "today it is a nice weather","you are best","I have a horse","you are my friend","buy, buy, buy!","it is spring in the air", "do you want to come?"};
		...
		String[] inputClasses=new String[listClasses.size()];
		Object[] obj2 = (Object[]) listClasses.toArray();
		for(int i=0; i<obj2.length;i++) {
			inputClasses[i] = (String)obj2[i];
		}
      
	  // String[] inputClasses = {"spam","spam","no spam","no spam","spam","no spam","no spam","spam","no spam","no spam"};
		BufferedReader testD=new BufferedReader(new FileReader("testText.txt"));
        String test=testD.readLine();
		while(test!=null)
		{
			listTest.add(test);
			test=testD.readLine();
		...
		String[] testText=new String[listTest.size()];
		Object[] obj3 = (Object[]) listTest.toArray();
		for(int i=0; i<obj3.length;i++) {
			testText[i] = (String)obj3[i];
		}
	//	String[] testText = {"Bengaluru Shopping Festival", "The Hindu Centre","usually I run in stairs", "buy it now!","buy, buy, buy!","you are the best, buy!","it is spring in the air"};
